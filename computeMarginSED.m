...
if (~isreal(Q) || ~isreal(Qvop))
    ...
    [M, xr] = computeMarginSED(CHtoRS(Q), CHtoRS(Qvop));
...
%             
%             
%             opt = optimset('Algorithm', 'sqp', 'MaxIter', 1000,  'Display', 'iter', 'GradObj', 'on', 'GradConstr', 'on');           
%             xi = fmincon(@(x) objfunMarginSAR(Qi,x), xi, [], [], [], [], [], [], @(x) constrfunMarginSAR(Qvop, x), opt);
%             Xi(:)=0;
%             Xi(:,1) = xi;  
%             
%             [Xi(I),fval1] = fmincon(@(x) objfun(Qi,x, I, J), Xi(I), [], [], [], [], [], [], @(x) constrfun(Qvop, x, I, J), opt);
%             
%             %opt = optimset('Algorithm', 'sqp', 'MaxIter', 1000,  'Display', 'iter', 'GradObj', 'on', 'GradConstr', 'on');
%             Xi = eye(Nc); Xi = Xi / SED(Qvop, Xi); 
%             [Xi(I),fval2] = fmincon(@(x) objfun(Qi,x, I, J), Xi(I), [], [], [], [], [], [], @(x) constrfun(Qvop, x, I, J), opt);
%             [i,fval1,fval2]
        end
        
        X(:,:,i) = Xi;
        M(i) = SED(Qvop,X(:,:,i))/SED(Qi,X(:,:,i)) - 1;
